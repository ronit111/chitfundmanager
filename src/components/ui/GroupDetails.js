...
export default function GroupDetails({ groupId, onBack, refreshGroups }) {
  ...
  useEffect(() => {
    ...
    // console.log('Current loading state at start of effect:', loading);
    
    // Always start with loading true when fetching
    setLoading(true);
    ...
    if (newGroupId) {
      async function fetchNewGroup() {
        // Loading is already set to true at the useEffect level
        try {
  ...
  // Add a separate effect to ensure loading is set to false when group data is available
  useEffect(() => {
    // If we have group data, make sure loading is false
    if (group) {
      // console.log('Group data is available, setting loading to false');
      setLoading(false);
    }
  }, [group]);
